...
{
    public class DapperUserStore : IApplicationUserStore
    {
        ...
        public DapperUserStore(IDbConnection conn)
        {
            var temp = conn ?? throw new ArgumentNullException(nameof(conn));

            _usersTable = new UsersTable(conn);
            _usersRolesTable = new UsersRolesTable(conn);
            _rolesTable = new RolesTable(conn);
            _usersClaimsTable = new UsersClaimsTable(conn);
            _usersLoginsTable = new UsersLoginsTable(conn);
        }
        ...
        {
            return this._usersClaimsTable.RemoveClaimsAsync(
                user, claims, cancellationToken);
        }

        public Task<IList<ApplicationUser>> GetUsersForClaimAsync(Claim claim, 
            CancellationToken cancellationToken)
        {
            return this._usersClaimsTable.GetUsersForClaimAsync(
